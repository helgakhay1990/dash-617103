...
src=src.slice(0,src.indexOf('const NUMB'));
const scope=new Function(src+'; return {DAYS, morningSteps};')();
const DAYS=scope.DAYS, morningSteps=scope.morningSteps;
const out=DAYS.map(d=>({
  d:d.d, wd:d.wd, n:d.n, t:d.t, kind:d.kind,
  dev:d.dev||null, min:d.min||0, boost:!!d.boost, air:!!d.air, last:!!d.last,
  ant:d.ant, act:d.act, meye:d.meye||null, note:d.note||null,
  steps:d.s.map(x=>({k:x.k,v:x.v})),
  msteps:morningSteps(d).map(x=>({k:x.k,v:x.v}))
}));
